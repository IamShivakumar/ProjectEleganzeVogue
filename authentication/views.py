...
from adminside.models import category, products,banners
    ...
    def post(self, request, email):
        ...
        if stored_data and str(stored_data.get("otp")) == otp:
            ...
            if user is not None:
                ...
                messages.success(
                    request,
                    f"User '{stored_data['username']}' has been created and logged in successfully!",
                )
                return redirect("homepage")
            else:
                messages.error(
                    request, "Error logging in. Please try to log in manually."
                )
...
class homepageview(TemplateView):
    def get(self, request):
        ...
        context = {"categories": categories,'banners':banner}
    ...
    if request.method == "GET":
        if request.user.is_admin:
            ...
            category_data = (
                ...
                )  # Adjust this to match your model relationships
                .annotate(total_quantity=Sum("quantity"))
                .order_by("-total_quantity")
            )
...
def chart_data(request):
    ...
    if filter == "daily":
        ...
        revenue_data = (
            Order.objects.filter(payment_status="Paid")
            .annotate(date=F("created_at__year"))
            .values("date")
            .annotate(total_revenue=Sum("total_price"))
            .order_by("date")
        )
